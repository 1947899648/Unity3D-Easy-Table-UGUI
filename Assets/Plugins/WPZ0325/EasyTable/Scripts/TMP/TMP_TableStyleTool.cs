...
namespace WPZ0325.EasyTable
{
    ...
    /// </summary>
    public class TMP_TableStyleTool : MonoBehaviour
    {
        [Header("----------------------TableType-----------------------------------------------")]
        ...
        public void SetTableColor()
        {
            ...
            {
                Image TMP_HeaderItem = m_HeaderItemHolder.GetChild(i).GetComponent<Image>();
                TMP_HeaderItem.color = IsOdd(i) ? HeaderItemOddColumnColor : HeaderItemEvenColumnColor;
            }
#if UNITY_EDITOR
            ...
            if (!Application.isPlaying)
            {
                ...
                {
                    Image TMP_ToggleRow = m_RowsHolderArea[0].content.GetChild(i).GetComponent<Image>();
                    if (TMP_ToggleRow != null)
                    {
                        TMP_ToggleRow.color = IsOdd(i) ? ToggleColumnOddRowColor : ToggleColumnEvenRowColor;
                    }
                }
                for (int i = 0; i < m_RowsHolderArea[1].content.childCount; i++)
                {
                    Image TMP_ButtonRow = m_RowsHolderArea[1].content.GetChild(i).GetComponent<Image>();
                    if (TMP_ButtonRow != null)
                    {
                        TMP_ButtonRow.color = IsOdd(i) ? ButtonColumnOddRowColor : ButtonColumnEvenRowColor;
                    }
                }
